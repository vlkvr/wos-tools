...
import { officerProjectData } from "@/data/officerProjectData";
import { type CalculatorItem } from "@/data/types";
import { safeEval, getConditionalBackgroundStyle } from "@/utils/calculations";
...
export default function OfficerProjectCalculator() {
  ...
  const loadInitialData = () => {
    const savedData = loadCalculatorData('OFFICER');
    return savedData || {
      stages: {
        stage1: {
          goal: "310000",
  ...
  useAutoSave('OFFICER', saveData, 500);
  ...
  const recalculateStage = (stageKey: string) => {
    const stage = stages[stageKey];
    const data = officerProjectData[stageKey as keyof typeof officerProjectData];
    ...
    data.forEach((item: CalculatorItem) => {
      ...
      const multiplier = item.type === "troops" ? (stage.troopLevels[item.id] || item.multiplier) : item.multiplier;
  ...
  const resetStage = (stageKey: string) => {
    const defaultGoal = stageKey === "stage1" ? "310000" : "453000";
    updateStage(stageKey, { 
      goal: defaultGoal,
      current: "",
      inputs: {},
      troopLevels: stageKey === "stage1" ? { "train-troops-1": 30 } : {},
  ...
  }, [stages.stage1?.goal, stages.stage1?.current, stages.stage1?.inputs, stages.stage1?.troopLevels, stages.stage1?.includePlanned, stages.stage2?.goal, stages.stage2?.current, stages.stage2?.inputs, stages.stage2?.troopLevels, stages.stage2?.includePlanned]);
              ...
              return (
                <tr key={item.id} className="hover:bg-wos-gray-50">
                  ...
                  <td className="border border-wos-gray-200 dark:border-border px-4 py-2 font-mono" data-testid={`required-${item.id}`}>
                    {required > 0 ? required.toLocaleString() : "—"}
                  </td>
                </tr>
              );
            })}
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="relative">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-transparent rounded-none p-0 h-auto gap-2 px-2 mb-0">
        ...
        <CardContent className="p-6">
          <div className="text-center">
            ...
            <p className="text-sm text-wos-gray-600 dark:text-muted-foreground mt-2">
              Combined points from all 2 stages
            </p>
          </div>
        </CardContent>
      </Card>
