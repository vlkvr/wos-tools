...
import { EscortRaidModal } from "@/components/modals/EscortRaidModal";
...
import { allianceShowdownData } from "@/data/allianceShowdownData";
...
export default function AllianceShowdownCalculator() {
  ...
  const [escortRaidModalOpen, setEscortRaidModalOpen] = useState(false);
  ...
  const loadInitialData = () => {
    const savedData = loadCalculatorData('ALLIANCE');
    return savedData || {
      stages: {
        stage1: {
          goal: "125000",
  ...
  useAutoSave('ALLIANCE', saveData, 500);
  ...
  const recalculateStage = (stageKey: string) => {
    const stage = stages[stageKey];
    const data = allianceShowdownData[stageKey as keyof typeof allianceShowdownData];
  ...
  const resetStage = (stageKey: string) => {
    ...
    if (stageKey === "stage4") {
      stageUpdates.troopLevels = { "train-troops-4": 24 };
    } else {
      stageUpdates.troopLevels = {};
    }
    
    updateStage(stageKey, stageUpdates);
  };
  ...
  useEffect(() => {
    ...
    stages.stage6?.goal, stages.stage6?.current, stages.stage6?.inputs, stages.stage6?.includePlanned,
  ]);
                      ...
                      {item.helpModal && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            } else if (item.helpModal === "escort-raid") {
                              setEscortRaidModalOpen(true);
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="relative">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-6 bg-transparent rounded-none p-0 h-auto gap-2 px-2 mb-0">
        ...
        <CardContent className="p-6">
          <div className="text-center">
            ...
            <p className="text-sm text-wos-gray-600 dark:text-muted-foreground mt-2">
              Combined points from all 6 stages
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {/* Escort/Raid Modal */}
      <EscortRaidModal
        isOpen={escortRaidModalOpen}
        onClose={() => setEscortRaidModalOpen(false)}
      />
