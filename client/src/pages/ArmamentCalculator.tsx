...
import { armamentData } from "@/data/armamentData";
...
export default function ArmamentCalculator() {
  ...
  // Helper function to colorize rare/epic/mythic words in labels
  ...
  const loadInitialData = () => {
    const savedData = loadCalculatorData('ARMAMENT');
    return savedData || {
      stages: {
        stage1: {
          goal: "20000",
  ...
  useAutoSave('ARMAMENT', saveData, 500);
  ...
  const recalculateStage = (stageKey: string) => {
    ...
    const data = armamentData[stageKey as keyof typeof armamentData];
    ...
    data.forEach((item: CalculatorItem) => {
      ...
      const earned = inputValue * item.multiplier;
      totalEarned += earned;
    });
  ...
  const resetStage = (stageKey: string) => {
    const defaultGoal = stageKey === "stage1" ? "20000" : "38000";
    updateStage(stageKey, { 
      goal: defaultGoal,
      current: "",
      inputs: {},
  ...
  }, [stages.stage1?.goal, stages.stage1?.current, stages.stage1?.inputs, stages.stage1?.includePlanned, stages.stage2?.goal, stages.stage2?.current, stages.stage2?.inputs, stages.stage2?.includePlanned]);
  ...
  const renderStageTable = (stageKey: string) => {
    ...
    const earnedValues = data.map((item: CalculatorItem) => {
      const inputValue = safeEval(stage.inputs[item.id]) || 0;
      return inputValue * item.multiplier;
    });
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {data.map((item: CalculatorItem) => {
              const inputValue = safeEval(stage.inputs[item.id]) || 0;
              const earned = inputValue * item.multiplier;
              const required = stage.remaining > 0 && item.multiplier > 0 
                ? Math.ceil(stage.remaining / item.multiplier) 
                : 0;
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-wos-gray-900 dark:text-foreground mb-2 flex items-center justify-center">
          ...
          Armament Competition Calculator
        </h1>
      </div>
